...
public class MIDISession {
    ...
    public void start() {
        Log.i(TAG, "MIDISession.start() called");
        if (isRunning) {
            Log.i(TAG, "MIDISession is already running");
            return;
        }
        setupNetworkListener();
        if(!isOnline()) {
            Log.d(TAG,"MIDI Start : not online, waiting for network...");
...
//        }
        this.netmask = getWifiAddress(); // Fallback to host address for now
    ...
    public void connect(final Bundle rinfo) {
        Log.d(TAG, "connect() called with rinfo: " + rinfo);
        if(isRunning) {
...
//        return false;
    ...
    public void sendUDPMessage(MIDIControl control, Bundle rinfo) {
        if(control != null && rinfo != null) {
            Log.d(TAG, "sendUDPMessage (Control): cmd=" + control.command + " to " + rinfo.getString(com.disappointedpig.midi.MIDIConstants.RINFO_ADDR) + ":" + rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT));

            int targetPort = rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT);
            int localPort = rinfo.getInt("local_port", 0);

            if (localPort == 5004 || (localPort == 0 && targetPort % 2 == 0)) {
                Log.d("MIDISession", "sendUDPMessage via controlChannel (5004) to rinfo:" + rinfo.toString());
                controlChannel.sendMidi(control, rinfo);
            } else {
                Log.d("MIDISession", "sendUDPMessage via messageChannel (5005) to rinfo:" + rinfo.toString());
...
//        Log.d("MIDISession","SyncronizeStoppedEvent");
    ...
    public void onPacketEvent(PacketEvent e) {
        Log.i(TAG, "onPacketEvent: Received " + e.getLength() + " bytes from " + e.getAddress().getHostAddress() + ":" + e.getPort());

        // try control first
        MIDIControl applecontrol = new MIDIControl();
        if(applecontrol.parse(e)) {
            Log.i(TAG, "onPacketEvent: Parsed as MIDI Control: command=" + applecontrol.command + " token=" + applecontrol.initiator_token + " ssrc=" + applecontrol.ssrc + " name=" + applecontrol.name);
            if(applecontrol.isValid()) {
                if(applecontrol.initiator_token != 0) {
                    ...
                    if (pending != null) {
                        Log.i(TAG, "onPacketEvent: Found pending stream for token " + applecontrol.initiator_token);
                ...
                if(stream == null) {
                    if (applecontrol.command == MIDIControl.AppleMIDICommand.INVITATION) {
                        Log.i(TAG, "onPacketEvent: Received NEW INVITATION from SSRC " + applecontrol.ssrc + " (" + applecontrol.name + ")");
                        stream = new MIDIStream();
                        streams.put(applecontrol.ssrc, stream);
                    } else {
                        Log.w(TAG, "onPacketEvent: Received control command " + applecontrol.command + " for unknown SSRC " + applecontrol.ssrc);
                        return;
                    }
                }
                ...
                Log.w(TAG, "onPacketEvent: Parsed control packet but it is invalid");
            }
        } else {
            MIDIMessage message = new MIDIMessage();
...
//    }
    ...
    public InetAddress getWifiAddress() {
        try {
            if (appContext == null) return InetAddress.getByName("127.0.0.1");
            ...
            while (interfaces.hasMoreElements()) {
                ...
                while (addresses.hasMoreElements()) {
                    ...
                    if (addr instanceof java.net.Inet4Address) {
                        Log.d(TAG, "Found IPv4 Address: " + addr.getHostAddress() + " on " + iface.getName());
                        return addr;
                    }
                }
            }
            ...
            Log.e(TAG, "Error getting WiFi address", e);
            try { return InetAddress.getByName("127.0.0.1"); } catch (Exception ex) { return null; }
        }
    }
    ...
    public boolean isOnline() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (capabilities != null) {
                android.net.NetworkCapabilities hc = cm.getNetworkCapabilities(capabilities);
                boolean connected = hc != null && (hc.hasTransport(android.net.NetworkCapabilities.TRANSPORT_WIFI) || hc.hasTransport(android.net.NetworkCapabilities.TRANSPORT_ETHERNET));
                Log.d(TAG, "isOnline? (Modern) " + (connected ? "ON" : "OFF"));
                return connected;
            }
        }
        ...
        Log.d(TAG, "isOnline? (Legacy) " + (connected ? "ON" : "OFF"));
        return connected;
    }
