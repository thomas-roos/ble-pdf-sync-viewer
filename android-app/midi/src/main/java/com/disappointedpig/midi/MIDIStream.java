...
//        invite.dumppacket();
    ...
    void handleControlMessage(MIDIControl control, Bundle rinfo) {
        Log.d(TAG, "handleControlMessage: cmd=" + control.command + " from " + rinfo.getString(com.disappointedpig.midi.MIDIConstants.RINFO_ADDR) + ":" + rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT));
        lastPacketReceivedTime = System.currentTimeMillis();

        switch(control.command) {
            case INVITATION:
                Log.d(TAG,"handle INVITATION from " + control.name + " SSRC=" + control.ssrc);
                handleInvitation(control,rinfo);
                resetCheckConnectionService();
                break;
            case INVITATION_ACCEPTED:
                Log.d(TAG,"handle INVITATION_ACCEPTED from " + control.name + " SSRC=" + control.ssrc);
                handleInvitationAccepted(control, rinfo);
                break;
            case INVITATION_REJECTED:
                Log.d(TAG,"handle INVITATION_REJECTED from " + control.name + " SSRC=" + control.ssrc);
            ...
            case RECEIVER_FEEDBACK:
                Log.d(TAG,"handle RECEIVER_FEEDBACK");
                break;
            case BITRATE_RECEIVE_LIMIT:
                Log.d(TAG,"handle BITRATE_RECEIVE_LIMIT");
                break;
            default:
                Log.d(TAG, "unhandled command: " + control.command);
                break;
        }
    }
    ...
    private void handleInvitation(MIDIControl control, Bundle rinfo) {
        Log.i(TAG, "handleInvitation: rinfo1=" + (rinfo1 == null ? "null" : rinfo1.toString()) + " rinfo2=" + (rinfo2 == null ? "null" : rinfo2.toString()));
        if(rinfo1 == null) {
            ...
            Log.i(TAG, "handleInvitation: Set rinfo1 to " + rinfo1.toString());
            ...
            Log.i(TAG, "handleInvitation: Received duplicate invitation on port " + rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT));
            // Still fall through to sendInvitationAccepted in case the first one was lost
        } else if(rinfo2 == null) {
            rinfo2 = (Bundle) rinfo.clone();
            this.isConnected = true;
            Log.i(TAG, "handleInvitation: Set rinfo2 to " + rinfo2.toString() + ". Stream is now CONNECTED.");
        } else {
            Log.w(TAG, "handleInvitation: Already have both rinfo1 and rinfo2. Ignoring.");
            return;
        }
...
//        EventBus.getDefault().post(new MIDIConnectionEndEvent(this.rinfo1));
    ...
    private void sendInvitationAccepted(Bundle rinfo) {
        Log.i(TAG, "sendInvitationAccepted to " + rinfo.getString(com.disappointedpig.midi.MIDIConstants.RINFO_ADDR) + ":" + rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT) + " (local_port=" + rinfo.getInt("local_port") + ")");
        ...
        if(!syncStarted && rinfo2 != null) {
            syncStarted = true;
            Log.i(TAG, "sendInvitationAccepted: Starting synchronization since rinfo2 is present");
            EventBus.getDefault().post(new SyncronizeStartedEvent(this.rinfo1));
        }
    ...
    private void sendInvitationRejected(Bundle rinfo) {
        Log.d(TAG, "sendInvitationRejected to " + rinfo.getString(com.disappointedpig.midi.MIDIConstants.RINFO_ADDR) + ":" + rinfo.getInt(com.disappointedpig.midi.MIDIConstants.RINFO_PORT));
        MIDIControl message = new MIDIControl();
